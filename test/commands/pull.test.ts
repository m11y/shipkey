...
// ---- Helper: replicate defaults fill logic from pull.ts ----
...
describe("pull defaults from shipkey.json", () => {
  ...
  test("fills missing defaults into .env", async () => {
    ...
    const count = await fillDefaults(envPath, {
      PORT: "3000",
    ...
    expect(content).not.toContain("PORT=3000 # generated by shipkey");
  });
  ...
  test("does not overwrite existing keys with defaults", async () => {
    ...
    const content = readFileSync(envPath, "utf-8");
    expect(count).toBe(1); // only LOG_LEVEL added
    expect(content).toContain("PORT=8080"); // original preserved
    expect(content).not.toContain("PORT=3000"); // default NOT applied
  ...
  test("repeated pulls do not duplicate defaults", async () => {
    ...
    await fillDefaults(envPath, defaults);
    await fillDefaults(envPath, defaults);
