...
describe("pull .env in-place merge with shipkey directives", () => {
  ...
  test("preserves comments, blank lines, and key ordering", async () => {
    ...
    expect(lines[1]).toBe("DATABASE_URL=postgresql://prod/mydb # shipkey: secret=true, managed=true");
    expect(lines[2]).toBe("PORT=3000");
    expect(lines[3]).toBe("");
    expect(lines[4]).toBe("# Third-party APIs");
    expect(lines[5]).toBe("STRIPE_KEY=sk_live_new # shipkey: secret=true, managed=true");
  });
  ...
  test("local-only keys are preserved untouched", async () => {
    ...
    expect(content).not.toContain("MY_LOCAL=debug_val # shipkey:");
    expect(content).toContain("API_KEY=new_val # shipkey: secret=true, managed=true");
  });

  test("removes managed keys that were deleted from the password manager", async () => {
    const envPath = join(TMP, ".env");
    writeFileSync(envPath, "OLD_KEY=old_val # shipkey: secret=true, managed=true\nLOCAL=x\n");
  ...
  test("removes legacy generated keys that were deleted from the password manager", async () => {
    const envPath = join(TMP, ".env");
  ...
  test("migrates legacy generated marker to shipkey directive on update", async () => {
    const envPath = join(TMP, ".env");
    writeFileSync(envPath, "API_KEY=old_val # generated by shipkey\n");
  ...
  test("preserves explicit secret=false when marking pulled keys as managed", async () => {
    const envPath = join(TMP, ".env");
    writeFileSync(envPath, "APNS_TEAM_ID=old # shipkey: secret=false\n");

    const existing = await readFile(envPath, "utf-8");
    const result = planEnvMerge(existing, [
      {
        ref: { vault: "shipkey", provider: "General", project: "myapp", env: "dev", field: "APNS_TEAM_ID" },
        value: "ABCDE12345",
      },
    ]);
    await writeFile(envPath, result.lines.join("\n"));
    ...
    const content = readFileSync(envPath, "utf-8");
    expect(content).toContain("APNS_TEAM_ID=ABCDE12345 # shipkey: secret=false, managed=true");
  });
  ...
  test("appends new keys from password manager not in local file", async () => {
    ...
    const content = readFileSync(envPath, "utf-8");
    expect(content).toContain("EXISTING=updated # shipkey: secret=true, managed=true");
    expect(content).toContain("BRAND_NEW=fresh # shipkey: secret=true, managed=true");
  ...
  test("creates .env from scratch when file does not exist", async () => {
    ...
    const content = readFileSync(envPath, "utf-8");
    expect(content).toContain("KEY_A=val_a # shipkey: secret=true, managed=true");
    expect(content).toContain("KEY_B=val_b # shipkey: secret=true, managed=true");
  });
